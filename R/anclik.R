fnPrepareANCLikelihoodData <- function(anc.prev, anc.n, anc.used = TRUE, anchor.year = 1970L, 
    return.data = TRUE) {
    ## anc.prev: matrix with one row for each site and column for each year anc.n:
    ## sample size, matrix with one row for each site and column for each year
    ## anchor.year: year in which annual prevalence output start -- to determine index
    ## to compare data NOTE: requires year to be stored in column names of anc.prev
    ...
    ## limit to years with both prevalence and N observations (likely input errors in
    ## EPP if not)
    ...
    anclik.dat <- list(W.lst = W.lst, v.lst = v.lst, n.lst = anc.n.lst, anc.idx.lst = anc.idx.lst)
    
    if (return.data) {
        ## Return the data matrices in the list (for convenience)
...
fnANClik <- function(qM, anclik.dat, v.infl = 0, s2.pr.alpha = 0.58, s2.pr.beta = 93, 
    VERSION = "C") {
    ## qM: vector of probit-transformed annual prevalences (starting in anchor.year
    ## specified for anclik.dat) anclik.dat: list including transformed ANC prevalence
    ## data v.infl: additional variance for ANC prevalence observation s2.pr.alpha:
    ## parameter for inverse-gamma prior on ANC site-level effects s2.pr.beta: parameter
    ## for inverse-gamma prior on ANC site-level effects
...
anc_resid_lik <- function(d.lst, v.lst, s2.pr.alpha = 0.58, s2.pr.beta = 93, VERSION = "C") {
    ...
    if (VERSION == "R") {
        ...
        return(integrate(Vectorize(function(s2) exp(sum(mapply(mvtnorm::dmvnorm, x = d.lst, 
            sigma = lapply(V.lst, function(m) s2 + m), MoreArgs = list(log = TRUE)))) * 
            s2^(-s2.pr.alpha - 1) * exp(-1/(s2.pr.beta * s2))), 1e-15, 0.3, subdivisions = 1000, 
...
sample.b.one <- function(d, v, s2.pr.alpha = 0.58, s2.pr.beta = 93) {
    ## Use rejection sampling to sample clinic level effect (Alkema, Raftery, Clark
    ## 2007) p(b.s | M, W.s) \propto N( d.st, v.st) * (b.s^2/2 + 1/beta2)^(-alpha-1/2)
    ## 1) sample from normal distribution with weighted mean and variance 2) reject
    ## based on second term of product
    
    max.val <- (1/s2.pr.beta)^(-s2.pr.alpha - 0.5)  # maximized when b=0, to normalize rejection sampling
    b <- Inf
    while (runif(1) > (0.5 * b^2 + 1/s2.pr.beta)^(-s2.pr.alpha - 0.5)/max.val) b <- rnorm(1, 
        sum(d/v)/sum(1/v), 1/sum(1/v))
    return(b)
}

sample.b.site <- function(qM, anclik.dat, s2.pr.alpha = 0.58, s2.pr.beta = 93) {
    ## Sample b.s values for all clinics parameters defined the same as fnANClik
...
sample.sigma2 <- function(b.site, s2.pr.alpha = 0.58, s2.pr.beta = 93) {
    ## Sample from posterior for variance of clinic random effects, conditional on
    ## sample of random effect values.
    
    ## b.site = matrix of b.site values (S x nsample) Note: scale parameterization, b0 =
    ## 1/s2.pr.beta p(sigma2 | b[1:S], a0, b0) ~ InvGamma(a0 + S/2, b0 + sum(b^2)/2)
    
    if (is.vector(b.site)) 
        b.site <- matrix(b.site)
    nsample <- ncol(b.site)
    nsites <- nrow(b.site)
    sigma2 <- 1/rgamma(nsample, shape = s2.pr.alpha + nsites/2, rate = 1/s2.pr.beta + 
        colSums(b.site^2)/2)
    return(sigma2)
}
...
sample.pred.site <- function(qM, b.site, anclik.dat, v.infl = 0) {
    ## Sample predicted prevalences in the same years as observed ANC prevalences in
    ## each site
    
    ## qM: vector of probit-transformed annual prevalences (starting in anchor.year
    ## specified for anclik.dat) b.site: vector of site-level random effects v.infl:
    ## additive variance term to (tranformed) binomial variance Note: b.site must be
    ## drawn from same posterior sample as qM (b.site | qM)!!!
    ...
    vpred.site <- mapply(function(pred, n) 2 * pi * exp(pred^2) * pnorm(pred) * (1 - 
        pnorm(pred))/n + v.infl, fit.site, anclik.dat$n.lst, SIMPLIFY = FALSE)
    
    ## predicted values (probit scale)
    pred.site <- mapply(rnorm, sapply(fit.site, length), fit.site, lapply(vpred.site, 
        sqrt), SIMPLIFY = FALSE)
